...
export default function createNewProject(arr){
    ...
    if (!title || !marker) {
      ...
      const projectInfo = createProjectObj(title, marker, date,UID);
      setStorageItem(`project${UID}`, projectInfo); //add to storage for later sessions;
